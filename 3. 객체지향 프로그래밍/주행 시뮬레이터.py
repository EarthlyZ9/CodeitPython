...
class DrivingSimulator:
    ...
    def add_vehicle(self, new_vehicle):
        ...
        if isinstance(new_vehicle, Vehicle) == True:
            self.vehiclelist.append(new_vehicle)
        else:
    ...
    def start_all_vehicles(self):
        ...
        return [i.start() for i in self.vehiclelist]  # list comprehension

    def stop_all_vehicles(self):
        """모든 교통 수단을 주행 정지시킨다"""
        print("모든 교통 수단을 주행 정지시킵니다!\n")
        return [i.stop() for i in self.vehiclelist]

    def __str__(self):
        """갖고 있는 교통 수단들의 현재 속도를 문자열로 리턴한다"""
        speed_all = ""
        for i in self.vehiclelist:
            speed_all = speed_all + str(i) + "\n"
        return speed_all
